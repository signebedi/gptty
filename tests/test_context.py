...
class TestContext(unittest.TestCase):
    ...
    def test_get_context_keywords_only(self):
        result = get_context("Tag1", 50, "tests/test_context_data.txt", context_keywords_only=True, question="Who is its mayor?")
        expected = "australia canberra 's capital city Who is its mayor?"
        self.assertEqual(result.strip(), expected.strip())

    def test_get_context_no_keywords(self):
        result = get_context("Tag1", 50, "tests/test_context_data.txt", context_keywords_only=False, question="Who is its mayor?")
